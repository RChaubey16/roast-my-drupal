 ...
 * @param characterMode Optional parameter to specify a character voice for the roast
 * @returns A formatted prompt string for generating roasts in standard or character-specific voice
 */
export const getRoastPrompt = (profileData: string, characterMode?: string) => {
  const basePrompt = `Create a hilarious roast of the Drupal profile data that I will provide you below. Be brutally funny but good-natured, using sarcasm, pop culture references, and movie analogies that would make even the most seasoned developer laugh.
  ...
  // If character mode is specified, add character-specific instructions
  if (characterMode) {
    return `${basePrompt}
  ...
  // Default prompt without character mode
  return `${basePrompt}
